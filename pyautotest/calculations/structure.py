...
LastEditTime: 2021-04-23 17:07:27
...
def toCartesian(positions, cell):
    """Transform direct coordinates to Cartesian format in unit bohr
...
def read_stru(ntype, stru_file=""):
    ...
    with open(stru_file, "r") as file:
        ...
        if re.match("ABFS_ORBITAL", file.readline()):
            for elem in elements:
        ...
        for elem in elements:
            if search_sentence(file, elem):
                ...
                for i in range(na):
                    ...
                    move_tmp.append(list(map(float, line.split()[3:])))
                if ctype == "Direct":
    ...
    return Stru(elements=elements, positions=positions, scaled_positions=scaled_positions, lat0=lat0, cell=cell, pps=pps, orbitals=orbitals, numbers=numbers, masses=masses, magmoms=magmoms, move=move, abfs=abfs)

class Stru:
    """ABACUS `STRU` file information"""

    def __init__(self, elements=[], positions={}, scaled_positions={}, lat0=None, cell=[], pps={}, orbitals={}, numbers={}, masses={}, magmoms={}, move={}, abfs={}) -> None:
        ...
        :params scaled_positions: dict, key is element name and value is list of atomic direct coordinates. This parameter had better not be set at the same time with `positions`. 
        ...
        self.lat0 = lat0
        self.cell = cell
        if positions and scaled_positions:
            raise TypeError("'positions' and 'scaled_positions' can not be set simultaneously")
        elif positions:
            ...
            self._ctype = "Direct"
            self.scaled_positions = scaled_positions
            self.positions = toCartesian(scaled_positions.copy(), self.cell)
        else:

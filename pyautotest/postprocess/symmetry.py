...
from typing import Counter, OrderedDict, Tuple, Union, Dict
...
class Spacegroup:
    ...
    def modified_stru(self, spgcell):
    ...
    def refine_cell(self, symprec:float=1e-5, angle_tolerance:float=-1) -> Union[Tuple[np.ndarray, np.ndarray, np.ndarray], None]:
    ...
    def find_primitive(self, symprec:float=1e-5, angle_tolerance:float=-1) -> Union[Tuple[np.ndarray, np.ndarray, np.ndarray], None]:
    ...
    def get_kpath(self, numbers:list=[], with_time_reversal:bool=True, recipe:str='hpkot', threshold:float=1e-7, symprec:float=1e-5, angle_tolerance:float=-1) -> dict:
        ...
        res = seekpath.get_path(self.spgcell, with_time_reversal, recipe, threshold, symprec, angle_tolerance)
        newcell = (res['primitive_lattice'], res['primitive_positions'], res['primitive_types'])
        ...
        kpt = Kpt(mode='Line', numbers=numbers, special_k=special_k, klabel=klabel)

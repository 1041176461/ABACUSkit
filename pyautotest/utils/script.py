...
def set_scheduler(
	...
	if scheduler == 'pbspro':
		from pyautotest.schedulers.plugins.pbsbaseclasses import PbsJobResource
		from pyautotest.schedulers.plugins.pbspro import PbsproScheduler
		...
		from pyautotest.schedulers.plugins.pbsbaseclasses import PbsJobResource
		from pyautotest.schedulers.plugins.torque import TorqueScheduler
		job_resource = PbsJobResource(num_machines=num_machines, num_mpiprocs_per_machine=num_mpiprocs_per_machine, num_cores_per_machine=num_cores_per_machine, num_cores_per_mpiproc=num_cores_per_mpiproc).resources
		sche = TorqueScheduler()

	elif scheduler == 'slurm':
		from pyautotest.schedulers.plugins.slurm import SlurmJobResource, SlurmScheduler
		...
		from pyautotest.schedulers.plugins.lsf import LsfJobResource, LsfScheduler
		job_resource = LsfJobResource(parallel_env=parallel_env, tot_num_mpiprocs=tot_num_mpiprocs).resources
		sche = LsfScheduler()

	elif scheduler == 'sge':
		from pyautotest.schedulers.plugins.sge import SgeJobResource, SgeScheduler
		job_resource = SgeJobResource(parallel_env=parallel_env, tot_num_mpiprocs=tot_num_mpiprocs).resources
		sche = SgeScheduler()

	elif scheduler == 'direct':
		from pyautotest.schedulers.plugins.direct import DirectJobResource, DirectScheduler

...
import numpy as np
from abacuskit.calculations.baseclass import ABACUSCalculation, JobCalculation
from abacuskit.calculations.plugins.scf import SCF
from abacuskit.calculations.structure import *
from abacuskit.schedulers.data import Code
from abacuskit.utils.IO import *
from abacuskit.utils.script import set_scheduler
from abacuskit.utils.test import profile
from abacuskit.utils.tools import *
...
class SetDimers(ABACUSCalculation):
    ...
    def __init__(self, input_dict: dict, stru: typing.Optional[Stru], kpt: typing.Optional[Kpt], Nu: list, dimer_num: int, **kwargs) -> None:
        ...
        if self.input_dict["basis_type"] not in ["lcao", "lcao_in_pw"]:
            raise KeyError(
                "ABFs based on lcao basis, so `basis_type` in 'INPUT' must be 'lcao' or 'lcao_in_pw'.")
    ...
    def _prepare(self, **kwargs):
        ...
        dis = cut_dis(cal_dis(self.stru.positions_bohr,
                      self.stru.supercell_positions(kpoints)), rcut_dict)
        ...
        for T1, T2 in dis_weight:
            for i_dis, i_weight in dis_weight[T1, T2].items():
                self.folder_weight[self.create_input(
                    T1, T2, i_dis, nw_dict)] = i_weight
    ...
    def create_input(self, T1: str, T2: str, i_dis: float, nw_list: dict) -> str:
        ...
        input_copy["nbands"] = (nw_list[T1] if abs(
            i_dis) < 1e-10 else nw_list[T1]+nw_list[T2])
        input_copy["nspin"] = 1
        ...
        if T1 == T2:
            ...
            orbitals = {T1: Path(self.stru.orbitals[T1]).name, T2: Path(
                self.stru.orbitals[T2]).name}
            masses = {T1: 1, T2: 1}
            magmoms = {T1: 0, T2: 0}
            positions = {T1: [[0.0, 0.0, 0.0]], T2: [[i_dis, 0.0, 0.0]]}
            move = {T1: [[0, 0, 0]], T2: [[0, 0, 0]]}
        obj = Stru(positions=positions, lat0=lat0, cell=cell, pps=pps,
                   orbitals=orbitals, masses=masses, magmoms=magmoms, move=move)
            ...
            if len(dis_tmp) <= self.dimer_num:
                ...
                if opt_mode == "linspace":
                    dis_opt[T1, T2] = np.linspace(
                        min(dis_tmp), max(dis_tmp), self.dimer_num)
    ...
    def cal_dis_weight(self, dis_opt: Dict_Tuple_Dict, dis_all: Dict_Tuple_Dict) -> Dict_Tuple_Dict:
        ...
        for T1, T2 in dis_opt:
            ...
            for index, i_dis_opt in enumerate(dis_opt_TT):
                ...
                i_dis_up = dis_opt_TT[index +
                                      1] if index < len(dis_opt_TT)-1 else np.infty
                for i_dis, num in dis_all[T1, T2].items():
                    if i_dis_low < i_dis < i_dis_opt:
                        i_weight += weight_func(i_dis_opt -
                                                i_dis, i_dis_opt-i_dis_low) * num
                    elif i_dis == i_dis_opt:
                        i_weight += num
                    elif i_dis_opt < i_dis < i_dis_up:
                        i_weight += weight_func(i_dis_opt -
                                                i_dis, i_dis_opt-i_dis_up) * num
    ...
    def _execute(self, command: Command, count: int = 1, **kwargs):
        ...
        subfolder_weight = {os.path.basename(
            key): value for key, value in self.folder_weight.items()}
        ...
        for folder in self.folder_weight:
            if not glob.glob(f"{folder}/matrix_*"):
                import warnings
                warnings.warn(
                    "'matrix_*' file not found in {folder}, it will execute again.")
        ...
        for folder in self.folder_weight:
            if not glob.glob(f"{folder}/matrix_*"):
                raise FileNotFoundError(
                    f"'matrix_*' file not found in {folder}, may be something wrong.")
    ...
    def set_matrix(self, folder_opt_matrix):
        for folder_matrix in self.folders_weight:
            ...
            shutil.copyfile(os.path.join(
                folder_matrix, matrix_file), copy_file)
            with open(copy_file, 'r') as file:
                nband = int(re.compile(
                    r"(\d+)\s+nbands").search(file.read()).group(1))
                self.folders_weight[folder_matrix] = (
                    self.folders_weight[folder_matrix], nband)

    def set_input(self):
        input_params = {
            # TODO: for new dpsi scripts, it should be a dict with key "origin"
            "file_list": [os.path.join("matrix", folder_matrix) for folder_matrix in self.folders_weight],
            "info":
            {
...
class EXX(SCF):
    ...
    def _prepare(self, **kwargs):
        ...
        self.obj_setdimers = SetDimers(deepcopy(self.input_dict), self.stru, self.kpt,
                                       self.Nu, self.dimer_num, **self.kwargs) if not Path("folders").exists() else None
        self.obj_optabfs = OptABFs(self.input_dict["exx_opt_orb_ecut"], self.stru, self.Nu, self.dr, self.lr,
                                   **self.kwargs) if not Path("opt_orb_"+"-".join(list_elem2str(self.Nu))).exists() else None
    ...
    def _execute(self, command: Code, external_command: Command = "", **kwargs):
        ...
        if not isinstance(command, Code):
            raise TypeError(
                "Type of `command` here must be `abacuskit.schedulers.data.Code` object")

...
class Code:
    ...
    def run_line(self):
        ...
        command_to_exec = f'{self.withmpi} '+' '.join(
            self.cmdline_params)+f' {self.code_name}' if self.withmpi else ' '.join(self.cmdline_params)
...
class JobDefaultFields:
    def __init__(self,
        ...
        # The first line of the submission script.
        self.shebang = shebang
        # if set, the job will be in a 'hold' status right after the submission.
        self.submit_as_hold = submit_as_hold
        # if the job is rerunnable (boolean).
        self.rerunnable = rerunnable
        # a dictionary with environment variables to set before the execution of the code
        self.job_environment = job_environment
        # the working directory for this job.
        self.working_directory = working_directory
        # an email address for sending emails on job events.
        self.email = email
        # if True, ask the scheduler to send an email when the job starts.
        self.email_on_started = email_on_started
        # if True, ask the scheduler to send an email when the job ends.
        self.email_on_terminated = email_on_terminated
        # the name of this job.
        self.job_name = job_name
        # a (relative) file name for the stdout of this job
        self.sched_output_path = sched_output_path
        # a (relative) file name for the stdout of this job
        self.sched_error_path = sched_error_path
        # if True, write both stdout and stderr on the same file (the one specified for stdout)
        self.sched_join_files = sched_join_files
        # the name of the scheduler queue (sometimes also called partition), on which the job will be submitted.
        self.queue_name = queue_name
        # the name of the scheduler account (sometimes also called projectid), on which the job will be submitted.
        self.account = account
        # the quality of service of the scheduler account, on which the job will be submitted.
        self.qos = qos
        # how many nodes and cpus it should use.
        self.job_resource = job_resource
        # a priority for this job.
        self.priority = priority
        # The maximum amount of memory the job is allowed to allocate ON EACH NODE, in kilobytes.
        self.max_memory_kb = max_memory_kb
        # The maximum wall clock time that all processes of a job are allowed to exist, in seconds.
        self.max_wallclock_seconds = max_wallclock_seconds
        # a string that will be inserted right after the last scheduler command, and before any other non-scheduler command.
        self.custom_scheduler_commands = custom_scheduler_commands
        # a (possibly multi-line) string to be inserted in the scheduler script before the main execution line.
        self.prepend_text = prepend_text
        # a (possibly multi-line) string to be inserted in the scheduler script after the main execution line.
        self.append_text = append_text
        # import the system environment variables.
        self.import_sys_environment = import_sys_environment
        # it contains the information necessary to run a single code.
        self.codes_run_mode = codes_run_mode
        # sets which the (multiple) codes have to be executed.

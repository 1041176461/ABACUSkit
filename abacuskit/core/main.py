...
from abacuskit.core.run import Run
from abacuskit.core.show import Show


def main():
    parser = argparse.ArgumentParser(
        prog='abacuskit', description='Auto-test and Postprocessing tools for ABACUS')
    ...
    test_group.add_argument('-s', '--single', dest='single', type=str,
                            default=None, help='local single test for one example.')
    test_group.add_argument('-b', '--batch', dest='batch', type=str,
                            default=None, help='local batch test for many examples.')

    other_group = parser_run.add_argument_group(title='Optional Parameters')
    other_group.add_argument('-l', '--local', dest='local', type=bool,
                             default=False, help='whether to run locally. Default: False')
    other_group.add_argument('-p', '--parallel', dest='parallel', type=bool, default=False,
                             help='whether to test in parallel. Only valid when running batch test non-locally. Default: False')
    parser_run.set_defaults(func=Run().run_cmdline)

    # Show
    parser_show = subparsers.add_parser(
        'show', help='show information of auto-test')
    parser_show.add_argument('-l', '--lib', dest='lib', type=str,
                             default=None, help='show example library information.')
    parser_show.add_argument('-b', '--band', dest='band', type=str,
                             default=None, help='plot band structure and show band information.')
    parser_show.add_argument('-d', '--dos', dest='dos', type=str,
